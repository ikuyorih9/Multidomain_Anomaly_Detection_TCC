...
import damicore.partition as p
import damicore.clustering as c

from damicore.tree import newick_format
from damicore._utils import frequency, normalize_list, dir_basename, open_outfile, get_version
from damicore.clustering import community_detection_names
...
def main(args): 
  ...
  #Now, by default, OS X multiprocessing starts processes with spawn instead of fork (like Windows). 
  #And with spawning, data is sent by pickling.
  ...
  if a.ncd_output:
    with open(a.ncd_output, 'wt') as f:
      if matrix_format == 'phylip':
        ...
        ncd.csv_write(f, ncd_results, write_header=True)
