...
from damicore.tree import Node, Leaf, Edge
def num_digits(x):
...
def update_distance_matrix(m, sums, i, j):#sums was a map, but now it is the list return by the map
...
def neighbor_joining(m, ids=None):
  ...
  for _ in range(n, 2, -1):
    ...
    m, di, dj = update_distance_matrix(m, s_list, i, j)
