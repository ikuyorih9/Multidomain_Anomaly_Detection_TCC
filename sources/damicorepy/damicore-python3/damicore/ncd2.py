...
import damicore.ncd_base as ncd_base
import damicore._utils as _utils
import damicore.compressor as c
from damicore.ncd_base import NcdResult, ncd
from damicore.datasource import create_factory
from damicore._utils import frequency, open_outfile
...
def chunk_indices(n, k, prng=Random()):
  ...
  #indices = range(n)# In Python3, output is: range(0, 10), thus it requires list(range(n))
  indices = list(range(n))
  prng.shuffle(indices)
  small, big = n // k, (n + k - 1) // k #n / k, (n + k - 1) / k # in the last version
  partition = []
  last = 0
  for i in range(n % k):# it was range(n % k): in Python2 last version
    partition.append(indices[last:last+big])
    last += big
  for i in range(k - n % k):# it was range(k - n % k): in Python2 last version
...
def parallel_compress_sources(sources, compressor, num_proc=mp.cpu_count()):
  ...
  pipes = [mp.Pipe() for _ in range(num_proc)]
...
def cli_parser():
  ...
  # print(f"\t[AVAILABLE COMPRESSORS]: {c.list_compressors()}")
  ...
  compressor_group.add_argument('-P', '--lossy', default=0, type=int, help="(webp) activate for lossy compression.")
  compressor_group.add_argument('--json-time', type=str, default=None, help="Caminho para salvar JSON com tempos de execução do compressor")
...
def parse_args(args):
  ...
  # print(f"\t ncd2.py: get compressor {args.compressor} with level={args.level}, model_order={args.model_order}, memory={args.memory} and restoration_method={args.restoration}")
