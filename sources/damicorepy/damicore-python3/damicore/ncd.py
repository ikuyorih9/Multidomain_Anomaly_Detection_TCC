...
def gzip_compression(fname, slowness = 6, **kwargs):
  """In-memory compression using the gzip library
  ...
  with io.BytesIO() as file:
    with gzip.open(file, mode= "wb",compresslevel=slowness) as f:
      f.write(data)
    file.seek(0)
    size = len(file.read())
  return size
...
def distance_matrix(directory, compression_name, pairing_name,
  ...
  if isinstance(directory, str): #in the last version: basestring):
